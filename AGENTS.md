...
| Removing spam that got in before the gate did | `mapsee_spam_purge.py` — reports by default, `--apply` to write. `spam-purge.yml` runs it daily at 07:40, BETWEEN the import and the janitor, and only REPORTS until the repository variable `SPAM_PURGE_APPLY` is `true` — the one switch that allows deletes, scheduled or manual |
